...
function getSystemStatus(){
    ...
    UsedSystemDisk=$(df -h | awk '$NF=="/"{printf "%dGB/%dGB (%s)\n", $3,$2,$5}')
...
function getHealthStatus(){
    ...
    cat /proc/loadavg |awk '{if ($3>=80) printf "警告：CPU总负载超过80, 当前负载 %s\n", $3}'
    ...
    [ -s /tmp/dmesg ] && grep -E '[C]all Trace' /tmp/dmesg && echo "警告：Call Trace happened" && grep -E '[C]all Trace' /tmp/dmesg
    more /var/log/messages|grep -E '[O]ut of memory' |awk '{for(i=3+1;i<=NF;i++)printf $i " ";printf"\n"}'|uniq
    [ -s /tmp/dmesg ] && grep -E '[O]ut of memory' /tmp/dmesg  && echo "警告：Out of memory happened" && grep -E '[O]ut of memory' /tmp/dmesg
...
function getProcessStatus(){
    ...
    if [ $(ps -ef | grep [d]efunct | wc -l) -ge 1 ];then
        echo ""
        echo "警告：存在僵尸进程：" ps -ef | grep [d]efunct ;
        echo "--------"
        ps -ef | head -n1
        ps -ef | grep [d]efunct
    fi
...
function getAgentStatus(){
    ...
    echo ""
    echo "        prometheus info:" `ps -ef|grep [t]ools/prometheus |awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
    echo "          kvmagent info:" `ps -ef|grep [k]vmagent|awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
    echo "         zsn-agent info:" `ps -ef|grep [z]sn-agent|awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
    echo "cephprimarystorage info:" `ps -ef|grep [c]ephprimarystorage|awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
    echo " cephbackupstorage info:" `ps -ef|grep [c]ephbackupstorage|awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
    echo "        zstack_tui info:" `ps -ef|grep [z]stack_tui|awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
    echo "      consoleproxy info:" `ps -ef|grep [c]onsoleproxy|awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
    echo "           dnsmasq info:" `ps -ef|grep [d]nsmasq |awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
    echo "          lighttpd info:" `ps -ef|grep [l]ighttpd |awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
    echo "          collectd info:" `ps -ef|grep [c]ollectd |awk '{for(i=7+1;i<=NF;i++)printf $i " ";printf"\n"}'`
